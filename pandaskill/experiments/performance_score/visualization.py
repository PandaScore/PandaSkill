...
def plot_all_models_calibration(
    ...
    for role in roles:
        ...
        label_formatted = label_dict.get(role, role) 
        ax1.plot(mean_predicted_value, fraction_of_positives, "s-",
                label="%s" % (label_formatted, ))

        ax2.hist(y_probs[role], range=(0, 1), bins=25, label=label_formatted,
                histtype="step", lw=2)
...
def plot_multiple_shap_features_impact(
    ...
    role_str_dict = { #TODO add to utils
        "top": "Top",
        "jungle": "Jungle",
        "mid": "Mid",
        "bottom": "Bot",
    ...
    for idx, role in enumerate(roles):        
        ...
        if idx > 0:
            ax.set_yticklabels([])
        ax.set_title(f"{role_str_dict[role]}")

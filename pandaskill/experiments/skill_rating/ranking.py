...
def evaluate_ranking(ranking: pd.DataFrame, experiment_dir: str) -> None:
    ...
    for ranking_kind in ["global", "europe", "north_america", "china", "korea"]:
        ...
        metrics[ranking_kind]["expert_eval"] = _evaluate_expert_against_model_and_other_experts(player_comparison_df, expert_names)
...
def _evaluate_expert_against_model_and_other_experts(player_comparison_df, expert_names) -> None:
    experts_metrics = {}
    for expert_index, expert_name in enumerate(expert_names):
        df_swapped_experts = player_comparison_df.copy()

        model_tmp = df_swapped_experts["model_player_0_is_better"].copy()
        df_swapped_experts["model_player_0_is_better"] = df_swapped_experts[f"expert_{expert_index}_player_0_is_better"]
        df_swapped_experts[f"expert_{expert_index}_player_0_is_better"] = model_tmp

        expert_metrics = _compute_concordance_metrics(df_swapped_experts, expert_names)
        experts_metrics[expert_name] = expert_metrics
    
    return experts_metrics

#TODO graph evolution of agreement based on rank difference between players
#TODO check meta ratings when computing end ranking, if a player has not played since the last meta rating update, it's meta rating isn't correct
